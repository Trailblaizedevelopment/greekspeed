...
Optional (Mapbox geocoding): `MAPBOX_SECRET_ACCESS_TOKEN` for `POST /api/geocoding/confirm`. Set `MAPBOX_GEOCODING_PERMANENT=false` only for local/dev without Mapbox billing; omit or `true` for stored results per [Mapbox permanent geocoding](https://docs.mapbox.com/api/search/geocoding/#storing-geocoding-results).
