...
| **Trailblaize donation drives** | App **`POST …/donations/campaigns`** only allows **`open`** or **`fundraiser`**. Same partner **`POST /chapters/:chapterId/collections`**: **`open`** → `type: "Payment"` + **`goalAmount` only** — **`requestedAmount` omitted** (`null` fails Crowded validation); **`fundraiser`** → `type: "Fundraising"` + `goalAmount` (**cents**) + `showOnPublicFundraisingChannels`. Per-payer fixed drives use **dues cycles**, not this table. **`data.uid`** / **`data.link`** normalized and stored. On Crowded **400**, the API route logs the error body for debugging. |
