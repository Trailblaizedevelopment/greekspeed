...
| **Trailblaize donation drives** | Same partner **`POST /chapters/:chapterId/collections`** with **`data`** under `buildCrowdedDonationCollectionRequest`: **`fixed`** → `type: "Payment"` + `requestedAmount` (cents); **`open`** → `type: "Payment"` + **`goalAmount` only** — **`requestedAmount` omitted** (sending `null` fails validation: must be a number); **`fundraiser`** → `type: "Fundraising"` + `goalAmount` (**cents**) + `showOnPublicFundraisingChannels`. List responses may still show `requestedAmount: null` for open. **`data.uid`** / **`data.link`** normalized and stored like other kinds. On Crowded **400**, the API route logs the error body for debugging. |
